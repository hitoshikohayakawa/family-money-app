-- Fix "column reference family_id is ambiguous" error in request_cashout_for_allowances.
-- The function's returns table defines family_id as an output variable, which conflicts
-- with the family_id column in the eligible CTE. Fix by aliasing the CTE in the
-- aggregate SELECT so the column reference is fully qualified.
...
begin
  ...
  select count(*), min(e.ag_family_id), sum(e.request_amount)
