-- Exclude allowance grants for disabled/removed children from guardian's view.
-- Previously the WHERE clause only checked ag.family_id = current_family_id,
-- so grants for children with status='disabled' were still returned.
-- Fix: require the child to have an active family_membership in the same family.
...
begin
  ...
  where ag.family_id = current_family_id
    ...
    -- This hides grants for disabled/removed children from the guardian's dashboard.
    -- Children viewing their own grants are always included (disabled children cannot
    -- log in due to the is_own_membership_disabled() login check).
    and (
