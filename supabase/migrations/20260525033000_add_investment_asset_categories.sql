alter table public.investment_assets
  add column if not exists asset_category_code text not null default 'index_stock';

update public.investment_assets
set asset_category_code = case asset_code
  when 'emaxis_slim_all_country' then 'index_stock'
  when 'toyota_motor_stock' then 'single_stock'
  when 'nintendo_stock' then 'single_stock'
  when 'sony_group_stock' then 'single_stock'
  else asset_category_code
end;
