-- Fix: rename local variable current_role → member_role to avoid conflict with
-- the PostgreSQL built-in current_role (returns the current database role name,
-- e.g. 'postgres'). Inside a security definer function current_role = 'postgres',
-- so the previous WHERE clause was always FALSE and returned 0 rows.
