...
begin
  ...
  select array_agg(distinct target_id)
    into normalized_ids
  from unnest(coalesce(target_allowance_grant_ids, array[]::uuid[])) as target_id
  ...
  with eligible as (
    select
      ag.id,
      ag.family_id as eligible_family_id,
      case
  ...
  select count(*), (array_agg(distinct eligible_family_id))[1], sum(request_amount)
  ...
  returning allowance_cashout_requests.id into new_request_id;
