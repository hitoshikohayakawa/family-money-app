-- Add avatar_path and avatar_emoji to list_family_members_for_current_user.
-- Both columns come from family_memberships which is already queried by this function.
-- Must DROP first because CREATE OR REPLACE cannot change a function's return type.

drop function if exists public.list_family_members_for_current_user();

create function public.list_family_members_for_current_user()
returns table (
  family_id    uuid,
  user_id      uuid,
  role         text,
