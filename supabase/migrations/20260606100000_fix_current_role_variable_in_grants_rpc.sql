-- Fix: rename PL/pgSQL variable `current_role` → `member_role` to avoid
-- shadowing the PostgreSQL built-in current_role function.
--
-- In the previous migration (20260604100000_filter_disabled_child_grants.sql)
-- the variable `current_role` was declared but, inside `return query`, PostgreSQL
-- evaluates `current_role` as the SQL built-in (returns the DB role, e.g.
-- 'authenticated'), not as the local variable. This caused the condition
-- `current_role in ('guardian_admin', 'guardian')` to be always FALSE, so
-- guardians received zero grants while children were unaffected.
...
declare
  ...
  member_role       text;   -- renamed from current_role to avoid SQL built-in conflict
begin
  ...
  where ag.family_id = current_family_id
    ...
    -- Only include grants for children who still have an active membership.
    and (
      ag.child_user_id = current_user_id
      or exists (
        select 1
        from public.family_memberships child_fms
        where child_fms.user_id   = ag.child_user_id
          and child_fms.family_id = current_family_id
