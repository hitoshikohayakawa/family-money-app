-- Fix "function min(uuid) does not exist" by splitting the aggregate query.
-- Get family_id via a separate LIMIT 1 lookup instead of min() on UUID.
...
begin
  ...
  -- Get family_id and granted_by_user_id from the first matching grant
  select ag.family_id, ag.granted_by_user_id
    into target_family_id, guardian_user_id
  from public.allowance_grants ag
  where ag.id = any(normalized_ids)
  limit 1;

  -- Count eligible (invested, owned by this child) grants and sum their amounts
  with eligible as (
    select
      ag.id,
      case
  ...
  select count(*), sum(e.request_amount)
    into target_count, total_amount
  from eligible e;
